...
public class ImageUtils {
    ...
    public static void saveCompressedImage(BufferedImage image, String toFileName, int type) {
        try {
            ...
            writer.setOutput(ImageIO.createImageOutputStream(new File(toFileName)));
     ...
     */
    private static BufferedImage createHeadlessSmoothBufferedImage(BufferedImage source, int type, int width, int height) {
        int imgType = (type == IMAGE_PNG && hasAlpha(source)) ? BufferedImage.TYPE_INT_ARGB : BufferedImage.TYPE_INT_RGB;
        BufferedImage dest = new BufferedImage(width, height, imgType);
        int sourcex, sourcey;
        ...
        for (int y = 0; y < height; y++) {
            sourcey = (int) (y * source.getHeight() / (double) dest.getHeight());
            ydiff = scale(y, scaley) - sourcey;
            for (int x = 0; x < width; x++) {
                sourcex = (int) (x * source.getWidth() / (double) dest.getWidth());
                xdiff = scale(x, scalex) - sourcex;
                x1 = (int) Math.min(source.getWidth() - 1, sourcex + 1);
                y1 = (int) Math.min(source.getHeight() - 1, sourcey + 1);
    ...
    private static int getRGBInterpolation(int value1, int value2, double distance) {
        ...
        int alpha = (int) ((alpha1 * (1.0 - distance) + alpha2 * distance) + 0.5);
        int red = (int) ((red1 * (1.0 - distance) + red2 * distance) + 0.5);
        int green = (int) ((green1 * (1.0 - distance) + green2 * distance) + 0.5);
        int blue = (int) ((blue1 * (1.0 - distance) + blue2 * distance) + 0.5);
